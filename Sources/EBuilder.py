...
class EBuilder(Entity):
    ...
    def start_building(self, buildingName):
        ...
        EntitySystem().get_entity(self.fakeBuilding).sprite = BuildingDatabase().get_building_sprite(buildingName)
        EntitySystem().get_entity(self.fakeBuilding).buildingName = buildingName
    ...
    def update(self):
        ...
        if UserInput().is_mouse_down() and self.buildingName != "":
            ...
            if (grid.is_inside(coord) and not grid.is_colliding(coord, BuildingDatabase().get_building_mask(self.buildingName))):
                buildingId = EntitySystem().add_entity(BuildingDatabase().get_building(self.buildingName))
